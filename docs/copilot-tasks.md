...
- [x] Task 1.1: Initialize Python Project
  - [x] Set up `pyproject.toml` (or `setup.cfg`) with name `shopify-nano-sre`, version `0.1.0`, Python >=3.11
  - [x] Create directory layout:
    - [x] `shopify-nano-sre/src/nano_sre/__init__.py`
    - [x] `shopify-nano-sre/src/nano_sre/cli.py`
    - [x] `shopify-nano-sre/src/nano_sre/agent/`
    - [x] `shopify-nano-sre/src/nano_sre/skills/`
    - [x] `shopify-nano-sre/src/nano_sre/db/`
    - [x] `shopify-nano-sre/src/nano_sre/config/`
    - [x] `shopify-nano-sre/tests/`
    - [x] `shopify-nano-sre/pyproject.toml`
    - [x] `shopify-nano-sre/.env.example`
    - [x] `shopify-nano-sre/README.md`
  - [x] Add dependencies: `playwright`, `litellm`, `rich`, `sqlite-utils`, `pydantic`, `pydantic-settings`, `click`, `httpx`, `pytest-asyncio`, `pytest-playwright`

- [x] Task 1.2: Create README.md with Positioning
  - [x] Hero tagline: "The open-source AI engineer that monitors your Shopify store 24/7."
  - [x] Feature badges (Python, Playwright, MIT License, etc.)
  - [x] Monitoring comparison table (Pingdom vs Datadog vs Nano-SRE)
  - [x] Quick-start install section (placeholder commands)
  - [x] "How it Works" diagram (Mermaid or ASCII)
  - [x] "Why Nano-SRE?" section highlighting Checkout Extensibility migration
  - [x] CTA to `shopintegrations.com` in footer

- [x] Task 1.3: Set Up Dev Tooling & CI
  - [x] `.github/workflows/ci.yml` (pytest, ruff, mypy)
  - [x] `.github/workflows/release.yml` (PyPI on tags)
  - [x] `ruff.toml` (config in pyproject.toml)
  - [x] `mypy.ini`
  - [ ] `.pre-commit-config.yaml` (ruff + mypy) - OPTIONAL
  - [x] `Dockerfile` (minimal Python image)

- [x] Task 1.4: Add License & Security Policy
  - [x] `LICENSE` (MIT)
  - [x] `SECURITY.md` (disclosure policy + contact)

---

## Group 2 — Core Agent Engine (parallel)

- [x] Task 2.1: Build the Configuration System
  - [x] Create `src/nano_sre/config/settings.py`
  - [x] Use `pydantic-settings` to load `.env` and CLI
  - [x] Fields: `store_url`, `shopify_admin_api_key` (optional), `llm_provider` (openai/anthropic/ollama), `llm_api_key`, `llm_model`, `alert_webhook_url`, `check_interval_minutes`, `sqlite_db_path`
  - [x] Validate URLs, sensible defaults
  - [x] `.env.example` with all fields documented

- [x] Task 2.2: Build the Agent Core Loop
  - [x] Create `src/nano_sre/agent/core.py`
  - [x] Agent loop: Trigger -> Observe -> Diagnose -> Act
  - [x] `Skill` base class with `name()` and `run(context) -> SkillResult`
  - [x] `SkillResult` dataclass: `status`, `summary`, `details`, `screenshots`
  - [x] Load skills, execute sequentially, collect results, diagnose, dispatch alerts
  - [x] Use `asyncio` throughout

- [x] Task 2.3: Set Up SQLite State Management
  - [x] Create `src/nano_sre/db/store.py` using `sqlite-utils`
  - [x] Tables: `check_runs`, `baselines`, `incidents`
  - [x] Helpers: `record_check()`, `get_latest_baseline()`, `update_baseline()`, `create_incident()`, `resolve_incident()`

- [x] Task 2.4: Add Scheduler + Webhook Trigger
  - [x] Create `src/nano_sre/agent/trigger.py`
  - [x] CRON-like interval scheduling for `watch`
  - [x] Optional webhook receiver for GitHub Deployment events
  - [x] Normalize trigger metadata into context

- [x] Task 2.5: Add Data Privacy + Redaction
  - [x] Create `src/nano_sre/agent/privacy.py`
  - [x] Redact secrets in logs/reports (API keys, tokens, emails)
  - [x] Optional `--redact` flag to blur PII selectors in screenshots
  - [x] Document data handling in README

---

## Group 3 — Skills Implementation (NEXT PRIORITY)
...
## Group 4 — Alerting & Reporting (NEXT PRIORITY)
...
## Group 5 — CLI & Packaging (SEQUENTIAL AFTER GROUP 3-4)
...
## Group 6 — LLM Integration (SEQUENTIAL AFTER GROUP 5)
...
## Group 7 — Testing (SEQUENTIAL AFTER GROUP 6)
...
## Group 8 — Documentation & Marketing Assets (AFTER ALL CODE GROUP)
